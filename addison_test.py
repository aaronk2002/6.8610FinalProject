...
# )
# mt.load_state_dict(torch.load("config/final.pth"))
# mt.to(config.device)
...
#         #     "accuracy", metrics["accuracy"], global_step=idx
#         # )
#         # train_summary_writer.add_scalar(
#         #     "learning_rate", scheduler.rate(), global_step=idx
#         # )
#         # train_summary_writer.add_scalar(
#         #     "iter_p_sec", end_time - start_time, global_step=idx
#         # )
...
#             #         "target_analysis", batch_y, global_step=e
#             #     )
#             #     train_summary_writer.add_histogram(
#             #         "source_analysis", batch_x, global_step=e
#             #     )
...
#             #     "loss", eval_metrics["loss"], global_step=idx
#             # )
#             # eval_summary_writer.add_scalar(
#             #     "accuracy", eval_metrics["accuracy"], global_step=idx
#             # )
#             # eval_summary_writer.add_histogram(
#             #     "logits_bucket", eval_metrics["bucket"], global_step=idx
#             # )
...
# torch.save(mt, args.model_dir + "/final2.pth")
...
print("\n\nNew mt\n")
new_mt = torch.load(args.model_dir + "/final2.pth")
new_mt.eval()
eval_x, eval_y = dataset.slide_seq2seq_batch(config.batch_size, config.max_seq, "eval")
eval_x = torch.from_numpy(eval_x).contiguous().to(config.device, dtype=torch.int)
eval_y = torch.from_numpy(eval_y).contiguous().to(config.device, dtype=torch.int)
